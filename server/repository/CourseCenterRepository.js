...
export const getCoursesbyCenter = async(params) =>{
    const sql = `select rno, course_id, statues, loc_id, mid_id, sub_id,
		course_name,course_start,price,apply_start,apply_end,course_sum, teacher_name, left(start_time,5) start_time, 
        ...
        from (		select row_number() over(order by c.course_id) as rno, 
                    ...
                    where loc_id = all(select distinct loc_id from location where loc_id = ?) and
						...
						date_format(start_time, '%p') = all(select distinct date_format(start_time,'%p') from course where date_format(start_time,'%p') = ?)
                        order by ?`
    return db
        .query(sql,[params.id, params.mid_id, params.sub_id,params.day, params.time,params.sort])
        .then(result=>result[0])
}
